...
public class AddUserActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_addUser.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                databaseManager = new DatabaseManager(AddUserActivity.this);
                testee_id = testee_name.getText().toString().trim()+"-"+testee_birthday.getText().toString().replace("/", "");
                databaseManager.addUser(new UserFormat(
                ...
                EmptyField();
                Toast.makeText(AddUserActivity.this,"User created successfully: ID = " + testee_id, Toast.LENGTH_SHORT).show();
            }
        });
    }
